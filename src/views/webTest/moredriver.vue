    ...
    return {
      ...
      (async function run() {  // https://www.bilibili.com/video/BV1154y1m7j1
          try {
              ...
              // 获取播放按钮
              let button1 = await driver.wait(until.elementLocated(By.className('bilibili-player-iconfont-start')), 2000);
              // 开始播放
              await button1.click()
              // 播放进行两秒
              await driver.sleep(2000)
              // 再次获取播放内部暂停按钮
              let button2 = await driver.wait(until.elementLocated(By.className('bilibili-player-video-wrap')), 2000);
              // 暂停播放
              await button2.click()
              // 等待一秒
